...
const COUNTERABLE_ACTIONS = [FOREIGN_AID, ASSASSINATE, STEAL]
...
const enumToString = (x) => {
    ...
    switch (x) {
        ...
        case BLOCK_AID:
            return 'Block Aid'
        case BLOCK_ASSASSINATE:
            return 'Block Assassinate'
        case BLOCK_STEAL_AMBASSADOR:
            return 'Block Steal (Ambassador)'
        case BLOCK_STEAL_CAPTAIN:
            return 'Block Steal (Captain)'
        case LOSE_INFLUENCE:
        ...
        players: [
            {
                ...
                lost_influence: [
                    ...game_state.players[0].lost_influence.map(ets),
                ],
                coins: game_state.players[0].coins,
            },
            {
                ...
                lost_influence: [
                    ...game_state.players[1].lost_influence.map(ets),
                ],
                coins: game_state.players[1].coins,
            },
        ],
...
const play_turn = (game_state) => {
    let s = game_state

    // Action
    let action = s.players[0].strategy(s, P_ACTION)
    s.history.push([0, ...action])

    // Challenge
    if (getRequiredCharacter(action[0])) {
        const challenge = s.players[1].strategy(
            shift_game_state(s),
            P_CHALLENGE
        )
        s.history.push([1, CHALLENGE, challenge])
        if (challenge) {
            const response = s.players[0].strategy(s, P_CHALLENGE_RESPONSE)
            ...
            if (response === getRequiredCharacter(action[0])) {
                ...
                s.players[1].hand.splice(
                    s.players[1].hand.indexOf(challenge),
                    1
                )
    ...
    let counter = null
    if (
        COUNTERABLE_ACTIONS.indexOf(action[0]) !== -1 &&
    ...
    if (counter) {
        ...
        if (challenge) {
            const response = s.players[1].strategy(
                shift_game_state(s),
                P_CHALLENGE_RESPONSE
            )
            ...
            if (response === getRequiredCharacter(counter)) {
                ...
                s.players[0].hand.splice(
                    s.players[0].hand.indexOf(challenge),
                    1
                )
    ...
    switch (action[0]) {
        ...
        case ASSASSINATE:
            ...
            const loss = s.players[1].strategy(
                shift_game_state(s),
                P_LOSE_INFLUENCE
            )
        ...
        case EXCHANGE:
            s.players[0].hand.push(s.deck.pop())
            s.players[0].hand.push(s.deck.pop())
            const discards = s.players[0].strategy(s, P_EXCHANGE_DISCARD)
...
const play_game = () => {
    ...
    while (true) {
        s = play_turn(s)
        if (
            s.players[0].lost_influence.length >= 2 ||
            s.players[1].lost_influence.length >= 2
        )
            break
        s = shift_game_state(play_turn(shift_game_state(s)))
        if (
            s.players[0].lost_influence.length >= 2 ||
            s.players[1].lost_influence.length >= 2
        )
